...
// 用于检测控制台是否被重写的正则对象（模块级复用，避免每次检测重复创建）
const devtoolsProbeRegex = /./;
...
function detectDevTools(): boolean {
  try {
    ...
    devtoolsProbeRegex.toString = function () {
      this.opened = true;
    };
    console.log('%c', devtoolsProbeRegex);
    // @ts-expect-error - 检查动态添加的属性
    if (devtoolsProbeRegex.opened) {
      return true;
    }
  } catch {
