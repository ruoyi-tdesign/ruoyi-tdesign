...
let confGlobal: Config | null = null;
...
function buildFormTemplate(conf: Config, child: string, type: string): string {
  let labelPosition = '';
...
function buildFromBtns(conf: Config, type: string): string {
  let str = '';
...
// span不为24的用el-col包裹
function colWrapper(element: Field, str: string): string {
...
const layouts: {
  [key: string]: (element: Field) => string;
} = {
  colFormItem(element: Field): string {
    let labelWidth = '';
  ...
  rowFormItem(element: Field): string {
...
const tags: {
  [key: string]: (el: Field) => string;
} = {
  'el-button': (el: Field): string => {
  ...
  'el-input-number': (el: Field): string => {
  ...
  'el-radio-group': (el: Field): string => {
  ...
  'el-checkbox-group': (el: Field): string => {
    const { disabled, vModel } = attrBuilder(el);
    const size = `size="${el.size}"`;
    const min = el.min ? `:min="${el.min}"` : '';
    const max = el.max ? `:max="${el.max}"` : '';
  ...
  'el-time-picker': (el: Field): string => {
    const { disabled, vModel, clearable, placeholder, width } = attrBuilder(el);
  ...
  'el-date-picker': (el: Field): string => {
    const { disabled, vModel, clearable, placeholder, width } = attrBuilder(el);
    const startPlaceholder = el['start-placeholder'] ? `start-placeholder="${el['start-placeholder']}"` : '';
    const endPlaceholder = el['end-placeholder'] ? `end-placeholder="${el['end-placeholder']}"` : '';
    const rangeSeparator = el['range-separator'] ? `range-separator="${el['range-separator']}"` : '';
  ...
  'el-color-picker': (el: Field): string => {
    const { disabled, vModel } = attrBuilder(el);
    const size = `size="${el.size}"`;
...
// el-buttin 子级
function buildElButtonChild(conf: Field): string {
...
// el-input innerHTML
function buildElInputChild(conf: Field): string {
  const children: string[] = [];
  if (conf.prepend) {
...
function buildElSelectChild(conf: Field): string {
  const children: string[] = [];
...
function buildElRadioGroupChild(conf: Field): string {
  const children: string[] = [];
  if (conf.options && conf.options.length) {
...
function buildElCheckboxGroupChild(conf: Field): string {
  const children: string[] = [];
  if (conf.options && conf.options.length) {
...
function buildElUploadChild(conf: Field): string {
  const list: string[] = [];
...
export function makeUpHtml(conf: Config, type: string): string {
