...
public class ActProcessInstanceServiceImpl implements IActProcessInstanceService {
    ...
    public boolean deleteRunInstance(ProcessInvalidBo processInvalidBo) {
        try {
            ...
            String deleteReason = LoginHelper.getUser().getNickname() + "作废了当前申请！";
            if (StringUtils.isNotBlank(processInvalidBo.getDeleteReason())) {
                deleteReason = LoginHelper.getUser().getNickname() + "作废理由:" + processInvalidBo.getDeleteReason();
            }
    ...
    public boolean cancelProcessApply(String businessKey) {
        try {
            ...
            for (Task task : taskList) {
                ...
                taskService.addComment(task.getId(), processInstanceId, TaskStatusEnum.CANCEL.getStatus(), LoginHelper.getUser().getNickname() + "：撤销申请");
            }
