...
public class SysTenantServiceImpl extends ServiceImpl<SysTenantMapper, SysTenant> implements ISysTenantService, TenantService {
    ...
    public Boolean insertByBo(SysTenantBo bo) {
        ...
        // 未开启工作流\未引入workflow模块，不执行下方操作
        if (workflowService != null &&
            SpringUtils.getProperty("warm-flow.enabled", Boolean.class, false)) {
                // 新增租户流程定义
    ...
    public void syncTenantConfig() {
        ...
        for (String tenantId : syncTenantIds) {
            TenantHelper.dynamicTenant(tenantId, () -> CacheUtils.clear(CacheNames.SYS_CONFIG));
        }
    }
